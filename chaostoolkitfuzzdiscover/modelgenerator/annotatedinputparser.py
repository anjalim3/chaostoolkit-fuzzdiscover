...
class AnnotatedInputParser:
    ...
    def __get_kittyfields_from_token(token):
        ...
        if "@@{ct_Fuzz_" in token and str(token).index("@@{ct_Fuzz_") == 0:
            ...
            if str(SupportedUserAnnotations.Number) in __unmarshalled_token and __unmarshalled_token.index(str(SupportedUserAnnotations.Number)) == 0:
                __unmarshalled_token = __unmarshalled_token.lstrip(str(SupportedUserAnnotations.Number)+"{").rstrip("}}")
                __kitty_fields.append(Float(value=float(__unmarshalled_token)))
            elif str(SupportedUserAnnotations.String) in __unmarshalled_token and __unmarshalled_token.index(str(SupportedUserAnnotations.String)) == 0:
                __unmarshalled_token = __unmarshalled_token.lstrip(str(SupportedUserAnnotations.String)+"{").rstrip("}}")
                __kitty_fields.append((String(value=__unmarshalled_token, fuzzable=True)))
            return __kitty_fields
        __kitty_fields.append(String(token, fuzzable=False))
        return __kitty_fields
